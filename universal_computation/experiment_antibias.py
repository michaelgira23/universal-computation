...
def experiment(
    ...
    dataset = AntiBiasDataset(batch_size=batch_size,eval_batch_size=eval_batch_size,model_name = model_name, input_max_dim = input_max_dim, device=device,vocab_size=vocab_size)
    ...
    for t in range(exp_args['num_iters']):
        ...
        if save_models and ((t+1) % exp_args['save_models_every'] == 0 or
            ...
            with open(model_path, 'wb') as f:
                state_dict = dict(model_state_dict=model.state_dict(), optimizer_state_dict=trainer.optim.state_dict())
...
def run_experiment(
    ...
    parser.add_argument('--num_iters', '-it', type=int, default=2,
    ...
    parser.add_argument('--model_path', '-mp', type=str, default='../universal_computation/saved_models/antibias.pt',
